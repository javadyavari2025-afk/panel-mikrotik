...
export function PeersChart({ stats, isLoading }: Props) {
  ...
  return (
    <Card className='border-border/60 relative col-span-1 flex flex-col overflow-hidden lg:col-span-2'>
      <div className='pointer-events-none absolute -top-16 -right-16 h-40 w-40 rounded-full bg-fuchsia-500/10 blur-3xl' />
      <CardHeader className='relative items-center pb-0'>
        <CardTitle>
          <h2 className='text-foreground mb-4 flex items-center gap-2 text-lg font-semibold'>
            <span className='h-5 w-1 rounded-full bg-gradient-to-b from-fuchsia-500 to-purple-500' />
            Users Statistics
          </h2>
        </CardTitle>
      </CardHeader>
      ...
      <CardContent className='relative flex-1 pb-0'>
        {!isLoading && isEmpty ? (
          <div className='text-muted-foreground flex h-[250px] flex-col items-center justify-center'>
